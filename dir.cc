...
void create(string const &name) {
  if (mkdir(name.c_str(), 0777) != 0) throw exCantCreate(name);
}

void remove(string const &name) {
  if (rmdir(name.c_str()) != 0) throw exCantRemove(name);
}
...
Listing::Listing(string const &dirName) : dirName(dirName) {
  ...
  if (!dir) throw exCantList(dirName);
}

Listing::~Listing() { closedir(dir); }
...
bool Listing::getNext(Entry &result) {
  ...
  for (;;) {
    if (readdir_r(dir, &entry, &entryPtr) != 0) throw exCantList(dirName);

    if (!entryPtr) return false;

#ifndef __APPLE__
    if (fstatat(dirfd(dir), entry.d_name, &entryStats, AT_SYMLINK_NOFOLLOW) !=
        0)
#else
    if (lstat(addPath(dirName, entry.d_name).c_str(), &entryStats) != 0)
#endif
