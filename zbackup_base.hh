...
class ZBackupBase : public Paths {
 public:
  ...
  DEF_EX(exWontReadFromTerminal, "Won't read data from a terminal",
         exInputError)
  ...
  DEF_EX_STR(exCantDeriveStorageDir,
             "The path must be within the backups/ dir:", Ex)
  ...
  /// Creates new storage
  static void initStorage(std::string const &storageDir,
                          std::string const &password, bool isEncrypted,
                          Config const &);

  /// For a given file within the backups/ dir in the storage, returns its
  /// storage dir or throws an exception
  static std::string deriveStorageDirFromBackupsFile(
      std::string const &backupsFile, bool allowOutside = false);
  ...
  bool spawnEditor(std::string &data,
                   bool (*validator)(string const &, string const &));
