...
TemporaryFile::~TemporaryFile() {
  if (!fileName.empty()) File::erase(fileName);
}

string const &TemporaryFile::getFileName() const { return fileName; }
...
sptr<TemporaryFile> TmpMgr::makeTemporaryFile() {
  ...
  if (fchmod(fd, S_IRUSR | S_IWUSR | S_IRGRP) != 0) throw exCantCreate(path);

  if (fd == -1 || close(fd) != 0) throw exCantCreate(path);
