...
void BundleCollector::finishIndex(string const &indexFn) {
  verbosePrintf(
      "Chunks used: %d/%d, bundles: %d kept, %d modified, %d removed\n",
  ...
  if (indexModified) {
    ...
    if (!config.runtime.gcConcat) chunkStorageWriter->reset();
...
void BundleCollector::finishBundle(Bundle::Id const &bundleId,
                                   BundleInfo const &info) {
