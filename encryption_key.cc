...
void deriveKey(string const &password, EncryptionKeyInfo const &info, void *key,
               unsigned keySize) {
...
void EncryptionKey::generate(string const &password, EncryptionKeyInfo &info,
                             EncryptionKey &encryptionkey) {
  ...
  info.set_key_check_hmac(
      calculateKeyHmac(key, sizeof(key), info.key_check_input()));

  // Encrypt the key
  AES_KEY aesKey;
  AES_set_encrypt_key((unsigned char const *)derivedKey, 128, &aesKey);
  char encryptedKey[sizeof(key)];
  AES_encrypt((unsigned char const *)key, (unsigned char *)encryptedKey,
              &aesKey);
