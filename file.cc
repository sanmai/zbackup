...
void File::erase(std::string const &filename) throw(exCantErase) {
  if (remove(filename.c_str()) != 0) throw exCantErase(filename);
}

void File::rename(std::string const &from,
                  std::string const &to) throw(exCantRename, exCantErase) {
  int res = 0;
...
/* Blast the bytes from one file to the other. */
      ...
      if (size == -1) throw exCantRename(from + " to " + to);
...
void File::open(char const *filename, OpenMode mode) throw(exCantOpen) {
  ...
  if (!f) throw exCantOpen(std::string(filename) + ": " + strerror(errno));
}
...
void File::open(int fd, OpenMode mode) throw(exCantOpen) {
  ...
  if (!f)
    throw exCantOpen("fd#" + Utils::numberToString(fd) + ": " +
                     strerror(errno));
}
...
File::File(std::string const &filename, OpenMode mode) throw(exCantOpen)
    : writeBuffer(0) {
  open(filename.c_str(), mode);
}

File::File(int fd, OpenMode mode) throw(exCantOpen) : writeBuffer(0) {
  open(fd, mode);
}
...
void File::read(void *buf, size_t size) throw(exReadError, exWriteError) {
  ...
  if (writeBuffer) flushWriteBuffer();
...
size_t File::readRecords(void *buf, size_t size,
                         size_t count) throw(exWriteError) {
  if (writeBuffer) flushWriteBuffer();
...
void File::write(void const *buf, size_t size) throw(exWriteError) {
  ...
  if (size >= WriteBufferSize) {
    ...
    if (result != 1) throw exWriteError();
  ...
  memcpy(writeBuffer + (WriteBufferSize - writeBufferLeft), buf, toAdd);
...
size_t File::writeRecords(void const *buf, size_t size,
                          size_t count) throw(exWriteError) {
  flushWriteBuffer();
...
char *File::gets(char *s, int size, bool stripNl) throw(exWriteError) {
  if (writeBuffer) flushWriteBuffer();
...
void File::seek(long offset) throw(exSeekError, exWriteError) {
  ...
  if (fseek(f, offset, SEEK_SET) != 0) throw exSeekError();
}

void File::seekCur(long offset) throw(exSeekError, exWriteError) {
  if (writeBuffer) flushWriteBuffer();

  if (fseek(f, offset, SEEK_CUR) != 0) throw exSeekError();
}

void File::seekEnd(long offset) throw(exSeekError, exWriteError) {
  if (writeBuffer) flushWriteBuffer();

  if (fseek(f, offset, SEEK_END) != 0) throw exSeekError();
}

void File::rewind() throw(exSeekError, exWriteError) { seek(0); }

size_t File::tell() throw(exSeekError) {
  long result = ftell(f);

  if (result == -1) throw exSeekError();

  if (writeBuffer) result += (WriteBufferSize - writeBufferLeft);
...
void File::close() throw(exWriteError) { fclose(release()); }
...
void File::flushWriteBuffer() throw(exWriteError) {
  if (writeBuffer && writeBufferLeft != WriteBufferSize) {
    size_t result =
        fwrite(writeBuffer, WriteBufferSize - writeBufferLeft, 1, f);
...
File::exReadErrorDetailed::exReadErrorDetailed(int fd) { buildDescription(fd); }
