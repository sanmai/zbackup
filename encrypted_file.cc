...
InputStream::InputStream(char const *fileName, EncryptionKey const &key,
                         void const *iv_)
    : file(fileName, UnbufferedFile::ReadOnly),
      filePos(0),
      key(key),
  ...
  if (key.hasKey()) {
    ...
    if (!size || size % BlockSize) throw exIncorrectFileSize();
  }
}
...
bool InputStream::Next(void const **data, int *size) {
  ...
  if (backedUp)
    ...
    try {
      ...
      start = buffer.data();
      fill =
          file.read(start + remainder, buffer.size() - remainder) + remainder;
...
int64_t InputStream::ByteCount() const { return filePos; }
...
void InputStream::checkAdler32() {
  ...
  Adler32::Value r;
  read(&r, sizeof(r));
  if (ours != fromLittleEndian(r)) throw exAdlerMismatch();
}
...
void InputStream::doDecrypt() {
  ...
  // support them
  CHECK(fill > 0 && !(fill % BlockSize),
        "incorrect size of the encrypted "
  ...
  memcpy(newIv, Encryption::getNextDecryptionIv(start, fill), sizeof(newIv));
  // Decrypt the data
...
OutputStream::OutputStream(char const *fileName, EncryptionKey const &key,
                           void const *iv_)
    : file(fileName, UnbufferedFile::WriteOnly),
      filePos(0),
      key(key),
      buffer(getPageSize()),
      start(buffer.data()),
  ...
  if (key.hasKey()) memcpy(iv, iv_, sizeof(iv));
}
...
bool OutputStream::Next(void **data, int *size) {
  ...
  if (backedUp)
    ...
    try {
      ...
      if (filePos) encryptAndWrite(buffer.size());
...
int64_t OutputStream::ByteCount() const { return filePos; }
...
void OutputStream::encryptAndWrite(size_t bytes) {
  if (key.hasKey()) {
    CHECK(bytes > 0 && !(bytes % BlockSize),
          "incorrect number of bytes to "
