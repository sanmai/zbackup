...
#define REQUIRE_VALUE                         \
  {                                           \
    if (!hasValue && !validate) return false; \
  }

#define PARSE_OR_VALIDATE(parse_src, validate_src) \
  (!validate && (parse_src)) || (validate && (validate_src))

DEF_EX_STR(exInvalidThreadsValue, "Invalid threads value specified:",
           std::exception)
...
void Config::prefillKeywords() {
  ...
  Keyword defaultKeywords[] = {
      // Storable options
      {"chunk.max_size", Config::oChunk_max_size, Config::Storable,
      ...
      {"bundle.max_payload_size", Config::oBundle_max_payload_size,
       Config::Storable,
      ...
      {"bundle.compression_method", Config::oBundle_compression_method,
       Config::Storable,
       "Compression method for new bundles\n"
       "Default is %s",
       GET_STORABLE(bundle, compression_method)},
      {"lzma.compression_level", Config::oLZMA_compression_level,
       Config::Storable,
      ...
      {"compression", Config::oBundle_compression_method, Config::Storable,
       "Shortcut for bundle.compression_method\n"
       "Default is %s",
       GET_STORABLE(bundle, compression_method)},

      // Runtime options
      {"threads", Config::oRuntime_threads, Config::Runtime,
      ...
      {"cache-size", Config::oRuntime_cacheSize, Config::Runtime,
      ...
      {"exchange", Config::oRuntime_exchange, Config::Runtime,
      ...
      {"gc.repack", Config::oRuntime_gcRepack, Config::Runtime,
  ...
  std::copy(defaultKeywords,
            defaultKeywords + sizeof(defaultKeywords) / sizeof(Keyword),
            keywords);
...
Config::~Config() {
  // prevent memleaks
  // TODO: use sptr
  if (cleanup_storable) delete storable;
  if (cleanup_keywords) delete[] keywords;
}

Config::Config() : cleanup_storable(true) {
...
Config::Config(ConfigInfo *configInfo) : cleanup_storable(false) {
  storable = configInfo;
  prefillKeywords();
  ...
  switch (opcode) {
    ...
    case oBundle_compression_method:
      ...
      if (PARSE_OR_VALIDATE(
        ...
        if (!lzma) {
          fprintf(stderr,
                  "zbackup is compiled without LZMA support, but the code "
                  "would support it. If you install liblzma (including "
                  "development files) "
                  "and recompile zbackup, you can use LZMA.\n");
          return false;
        }
        Compression::CompressionMethod::selectedCompression = lzma;
      } else if (PARSE_OR_VALIDATE(
                     strcmp(optionValue, "lzo1x_1") == 0 ||
                         strcmp(optionValue, "lzo") == 0,
        ...
        if (!lzo) {
          fprintf(stderr,
                  "zbackup is compiled without LZO support, but the code "
                  "would support it. If you install liblzo2 (including "
                  "development files) "
    ...
    case oRuntime_cacheSize:
      ...
      if (sscanf(optionValue, "%zu %15s %n", &sizeValue, suffix, &n) == 2 &&
          !optionValue[n]) {
        runtime.cacheSize = sizeValue * Utils::getScale(suffix);
        if (0 == runtime.cacheSize) return false;
    ...
    case oRuntime_gcRepack:
      if (runtime.gcConcat) {
        fprintf(stderr,
                "Option gc.repack is mutually exclusive with gc.concat "
    ...
    case oRuntime_gcConcat:
      if (runtime.gcRepack) {
        ...
        fprintf(stderr,
                "Option gc.concat is mutually exclusive with gc.repack "
    ...
    case oRuntime_backupMinimalSize:
      ...
      if (sscanf(optionValue, "%zu %15s %n", &sizeValue, suffix, &n) == 2 &&
        ...
        if (0 == runtime.backupMinimalSize) return false;
...
void Config::showHelp(const OptionType type) {
  ...
  for (u_int i = 0; !keywords[i].name.empty(); i++) {
    if (keywords[i].type != type) continue;
...
bool Config::parseProto(const string &str,
...
bool Config::validateProto(const string &oldConfigData,
                           const string &configData) {
  Config config;
  dPrintf("Validating proto...\n");
  if (!parseProto(configData, config.storable)) return false;
...
void Config::reset_storable() {
  ...
  SET_STORABLE(bundle, max_payload_size,
               defaultConfig.GET_STORABLE(bundle, max_payload_size));
...
void Config::show() { printf("%s", toString(*storable).c_str()); }
