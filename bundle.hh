...
struct Id {
  ...
  bool operator!=(Id const &other) const { return !operator==(other); }
  bool operator<(Id const &other) const {
    return memcmp(blob, other.blob, sizeof(blob)) < 0;
  }
};
...
class Reader : NoCopy {
 ...
 public:
  ...
  DEF_EX(exUnsupportedVersion, "Unsupported version of the index file format",
         Ex)
  ...
  BundleInfo getBundleInfo() { return info; }
  BundleFileHeader getBundleHeader() { return header; }
...
class Creator {
 ...
 public:
  ...
  BundleInfo const &getCurrentBundleInfo() const { return info; }
};
...
/// already
string generateFileName(Id const &, string const &bundlesDir, bool createDirs);
}
