...
vector<string> findOrRebuild(string const &src, string const &dst,
                             string const &relativePath) {
  vector<string> files;
  ...
  while (lst.getNext(entry)) {
    ...
    if (currentRelativePath.empty())
      ...
      currentRelativePath.assign(
          Dir::addPath(relativePath, entry.getFileName()));
...
unsigned int getScale(char *suffix) {
  ...
  // Check the suffix
  for (char *c = suffix; *c; ++c) *c = tolower(*c);
