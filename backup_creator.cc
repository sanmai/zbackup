...
BackupCreator::BackupCreator(Config const &config, ChunkIndex &chunkIndex,
                             ChunkStorage::Writer &chunkStorageWriter)
    : chunkMaxSize(config.GET_STORABLE(chunk, max_size)),
      chunkIndex(chunkIndex),
      chunkStorageWriter(chunkStorageWriter),
      ringBufferFill(0),
      chunkToSaveFill(0),
      backupDataStream(
          new google::protobuf::io::StringOutputStream(&backupData)),
...
void *BackupCreator::getInputBuffer() { return head; }

size_t BackupCreator::getInputBufferSize() {
  if (tail > head)
    return tail - head;
  else if (tail == head && ringBufferFill)
    return 0;
  else
    return end - head;
}
...
void BackupCreator::saveChunkToSave() {
  ...
  if (chunkToSaveFill < 128)  // TODO: make this value configurable
  {
    ...
    id.rollingHash = RollingHash::digest(chunkToSave.data(), chunkToSaveFill);
    unsigned char sha1Value[SHA_DIGEST_LENGTH];
    SHA1((unsigned char const *)chunkToSave.data(), chunkToSaveFill, sha1Value);
...
void BackupCreator::finish() {
  ...
  CHECK(chunkToSaveFill + ringBufferFill <= chunkMaxSize,
        "had more than two "
        "full chunks at backup finish");

  moveFromRingBufferToChunkToSave(ringBufferFill);

  if (chunkToSaveFill) saveChunkToSave();
}
