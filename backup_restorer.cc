...
void restoreMap(ChunkStorage::Reader &chunkStorageReader,
  ...
  string chunk;
  size_t chunkSize;
  for (ChunkMap::const_iterator it = chunkMap->begin(); it != chunkMap->end();
       it++) {
    for (ChunkPosition::const_iterator pi = (*it).second.begin();
         pi != (*it).second.end(); pi++) {
      if (output) {
...
void restore(ChunkStorage::Reader &chunkStorageReader,
             std::string const &backupData, DataSink *output,
             ChunkSet *chunkSet, ChunkMap *chunkMap, SeekableSink *seekOut) {
  ...
  while (cis.BytesUntilLimit() > 0) {
    ...
    if (instr.has_chunk_to_emit()) {
      ...
      if (chunkMap) {
        ...
        if (it == chunkMap->end()) {
          ChunkPosition v;
          std::pair<ChunkMap::iterator, bool> r =
              chunkMap->insert(std::make_pair(*bundleId, v));
          it = r.first;
        }
    ...
    if ((output || chunkMap) && instr.has_bytes_to_emit()) {
      ...
      if (chunkMap) {
        if (seekOut) seekOut->saveData(position, bytes.data(), bytes.size());
        position += bytes.size();
      }
    }
  }
...
void restoreIterations(ChunkStorage::Reader &chunkStorageReader,
                       BackupInfo &backupInfo, std::string &backupData,
                       ChunkSet *chunkSet) {
  ...
  for (;;) {
    ...
    if (backupInfo.iterations()) {
      ...
      restore(chunkStorageReader, backupData, &stringWriter, chunkSet, NULL,
              NULL);
...
class BackupInstructionsIterator : NoCopy {
 public:
  ...
  ~BackupInstructionsIterator() { cis.PopLimit(limit); }
...
int64_t IndexedRestorer::size() const { return totalSize; }
...
void IndexedRestorer::saveData(int64_t offset, void *data, size_t size) const {
  if (offset < 0 || offset + size > totalSize) throw exOutOfRange();

  // Find first instruction which generates output range that starts after
  // offset
