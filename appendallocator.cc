...
AppendAllocator::AppendAllocator(unsigned blockSize_, unsigned granularity)
    : alignMask(granularity - 1),
      ...
      // bytes
      blockSize((blockSize_ + alignMask) & ~alignMask),
      leftInBlock(-1) {}
...
char *AppendAllocator::allocateBytes(unsigned size) {
  ...
  if (!size && !blocks.empty()) return nextAvailable;
...
void AppendAllocator::returnBytes(unsigned size) {
  ...
  if (nextAvailable == blocks.back().data) {
    if (blocks.size() == 1) throw std::bad_alloc();
  ...
  if (nextAvailable - blocks.back().data < (int)toTake) throw std::bad_alloc();
...
void AppendAllocator::clear() {
  for (unsigned x = blocks.size(); x--;) free(blocks[x].data);
  blocks.clear();

  leftInBlock = -1;
}

AppendAllocator::~AppendAllocator() { clear(); }
