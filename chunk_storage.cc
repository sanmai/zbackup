...
Writer::Writer(Config const &configIn, EncryptionKey const &encryptionKey,
    ...
    : config(configIn),
      ...
      maxCompressorsToRun(maxCompressorsToRun),
      runningCompressors(0) {
  verbosePrintf("Using up to %zu thread(s) for compression\n",
                maxCompressorsToRun);
}

Writer::~Writer() { waitForAllCompressorsToFinish(); }
...
void Writer::addBundle(BundleInfo const &bundleInfo,
                       Bundle::Id const &bundleId) {
  if (!indexFile.get()) {
    // Create a new index file
    indexTempFile = tmpMgr.makeTemporaryFile();
    indexFile =
        new IndexFile::Writer(encryptionKey, indexTempFile->getFileName());
  }
...
void Writer::commit() {
  ...
  for (size_t x = pendingBundleRenames.size(); x--;) {
    ...
    r.first->moveOverTo(Bundle::generateFileName(r.second, bundlesDir, true));
  }
  ...
  if (indexFile.get()) {
    ...
    indexTempFile->moveOverTo(
        Dir::addPath(indexDir, Utils::toHex(buf, sizeof(buf))));
    indexTempFile.reset();
  }
}
...
Bundle::Creator &Writer::getCurrentBundle() {
  if (!currentBundle.get()) currentBundle = new Bundle::Creator;
  return *currentBundle;
}

void Writer::finishCurrentBundle() {
  if (!currentBundle.get()) return;
  ...
  Compressor *compressor =
      new Compressor(config, *this, currentBundle, file->getFileName());
...
Reader::Reader(Config const &configIn, EncryptionKey const &encryptionKey,
    ...
    : config(configIn),
      ...
      // in the same bundle
      cachedReaders(maxCacheSizeBytes <
                            config.GET_STORABLE(bundle, max_payload_size)
                        ? 1
                        : maxCacheSizeBytes /
                              config.GET_STORABLE(bundle, max_payload_size)) {
...
Bundle::Id const *Reader::getBundleId(ChunkId const &chunkId, size_t &size) {
  ...
  if (Bundle::Id const *bundleId = index.findChunk(chunkId, &s)) {
    ...
    throw exNoSuchChunk(
        Utils::toHex((unsigned char const *)blob.data(), blob.size()));
  }
}
...
Bundle::Reader &Reader::getReaderFor(Bundle::Id const &id) {
  ...
  if (!reader.get()) {
    // Load the bundle
    reader = new Bundle::Reader(Bundle::generateFileName(id, bundlesDir, false),
                                encryptionKey);
  }
