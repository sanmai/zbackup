...
class LZMAEnDecoder : public EnDecoder {
 ...
 public:
  ...
  size_t getAvailableInput() { return strm.avail_in; }

  size_t getAvailableOutput() { return strm.avail_out; }

  bool process(bool finish) {
    lzma_ret ret = lzma_code(&strm, (finish ? LZMA_FINISH : LZMA_RUN));

    CHECK(ret == LZMA_OK || ret == LZMA_STREAM_END, "lzma_code error: %d",
          (int)ret);

    return (ret == LZMA_STREAM_END);
  }

  ~LZMAEnDecoder() { lzma_end(&strm); }
};
...
class LZMACompression : public CompressionMethod {
 public:
  ...
  sptr<EnDecoder> createEncoder() const { return new LZMAEncoder(); }

  sptr<EnDecoder> createDecoder() const { return new LZMADecoder(); }
...
class NoStreamEnDecoder : public EnDecoder {
 ...
 protected:
  ...
  virtual bool shouldTryWith(const char *dataIn, size_t availIn,
                             size_t availOut) = 0;
  ...
  virtual bool doProcess(const char *dataIn, size_t availIn, char *dataOut,
                         size_t availOut, size_t &outputSize) = 0;
 ...
 public:
  ...
  size_t getAvailableInput() { return availIn; }

  size_t getAvailableOutput() { return availOut; }
  ...
  bool process(bool finish) {
    ...
    // If the input has been processed, try to copy some of it to the output
    // buffer.
    if (processed) {
 ...
 private:
  void processFinish(const char *dataIn, size_t availIn) {
    ...
    do {
      ...
      if (doProcess(dataIn, availIn, (char *)accDataOut.data(), bufferSize,
                    outputSize)) {
...
//     malicious person and there is nothing I can do about it. I could check
//     for
...
class NoStreamAndUnknownSizeDecoder : public NoStreamEnDecoder {
 protected:
  ...
  bool isCompleteInput(const char *dataIn, size_t availIn) {
    if (availIn < 2 * sizeof(uint64_t)) return false;
  ...
  size_t suggestOutputSize(const char *dataIn, size_t availIn) {
    ...
    // We're not using size_t because we need a type that has the same size on
    // all
    ...
    // NOTE If your compiler doesn't accept this cast, your size_t is smaller
    // than
  ...
  bool doProcess(const char *dataIn, size_t availIn, char *dataOut,
                 size_t availOut, size_t &outputSize) {
    if (availIn < 2 * sizeof(uint64_t)) return false;

    // NOTE We skip 8 bytes. If we later decide to drop compatibility with
    // 32-bit
    ...
    if (!doProcessNoSize(dataIn, availIn, dataOut, availOut,
                         reportedOutputSize))
      return false;
...
class NoStreamAndUnknownSizeEncoder : public NoStreamEnDecoder {
 protected:
  ...
  size_t getOverhead() { return 2 * sizeof(uint64_t); }
  ...
  bool doProcess(const char *dataIn, size_t availIn, char *dataOut,
    ...
    CHECK(availIn <= UINT32_MAX,
          "You want to compress more than 4GB of data?! "
    ...
    *(uint32_t *)dataOut = htole32(availIn);
    ...
    // We skip more than we actually use; see
    // NoStreamAndUnknownSizeDecoder::doProcess(...).
    dataOut += getOverhead();
    ...
    CHECK(outputSize <= UINT32_MAX,
          "The compressed data is more than 4GB?! "
          "Sorry, we don't support that, yet.");
...
class LZO1X_1_Decoder : public NoStreamAndUnknownSizeDecoder {
 protected:
  bool doProcessNoSize(const char *dataIn, size_t availIn, char *dataOut,
                       size_t availOut, size_t &outputSize) {
    // same argument is used for available output size and size of decompressed
    // data
    outputSize = availOut;
    int ret =
        lzo1x_decompress_safe((const lzo_bytep)dataIn, availIn,
                              (lzo_bytep)dataOut, (lzo_uintp)&outputSize, NULL);

    if (ret == LZO_E_OUTPUT_OVERRUN) return false;
...
class LZO1X_1_Compression : public CompressionMethod {
 ...
 public:
  ...
  lzo_voidp getWorkmem(size_t size) const { return new char[size]; }
...
size_t LZO1X_1_Encoder::calcMaxCompressedSize(size_t availIn) {
  // It seems that lzo1x_1_compress does NOT check whether the buffer is big
  // enough.
...
bool LZO1X_1_Encoder::shouldTryWith(const char *dataIn, size_t availIn,
                                    size_t availOut) {
...
bool LZO1X_1_Encoder::doProcessNoSize(const char *dataIn, size_t availIn,
  ...
  // It seems that lzo1x_1_compress does NOT check whether the buffer is big
  // enough.
  // Therefore, we won't try it unless we are sure that the buffer is big
  // enough.
  if (availOut < calcMaxCompressedSize(availIn)) return false;
  ...
  int ret =
      lzo1x_1_compress((const lzo_bytep)dataIn, availIn, (lzo_bytep)dataOut,
                       (lzo_uintp)&outputSize, wrkmem);
...
class ZeroEnDecoder : public EnDecoder {
 ...
 public:
  ZeroEnDecoder() { BackUp = 0; }

  size_t getAvailableInput() { return BackUp; }

  size_t getAvailableOutput() { return BackUp; }
};
...
class ZeroEncoder : public ZeroEnDecoder {
  ...
  bool process(bool finish) {
    ...
    if (0 >= left) {
      if (toCopy < size) BackUp = size - toCopy;
      return true;
    }
...
class ZeroCompression : public CompressionMethod {
 public:
  ...
  sptr<EnDecoder> createEncoder() const { return new ZeroEncoder(); }

  sptr<EnDecoder> createDecoder() const { return new ZeroDecoder(); }
...
bool CompressionMethod::iterator::atEnd() const { return !ptr || !*ptr; }
...
CompressionMethod::iterator CompressionMethod::end() { return iterator(NULL); }
}
