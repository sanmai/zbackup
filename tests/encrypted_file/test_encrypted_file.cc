...
void readAndWrite(EncryptionKey const &key, bool writeBackups, bool readBackups,
                  bool readSkips) {
  ...
  fprintf(stderr, "Run with %d bytes, %s%s%s%sfile %s...\n", fileSize,
          ...
          readBackups ? "read backups, " : "", readSkips ? "read skips, " : "",
  ...
  // Write
  {
    ...
    for (int left = fileSize; left;) {
      CHECK(out.ByteCount() == fileSize - left,
            "Incorrect bytecount in the "
            "middle of writing");
      void *data;
      ...
      if (doBackup) {
        ...
        // the write
        if (avail > left) backup = avail - left;
        avail -= backup;
      }
    ...
    if (avail || (rand() & 1)) out.BackUp(avail);
  ...
  // Read back
  {
    ...
    for (int left = fileSize; left;) {
      ...
      CHECK(in.ByteCount() == fileSize - left,
            "Incorrect bytecount in the "
            "middle of reading");
      ...
      CHECK(memcmp(next, data, toRead) == 0,
            "Different bytes read than "
            "expected at offset %d",
            int(next - rnd));

      if (doBackup) in.BackUp(backup);
    ...
    CHECK(!in.Next(&data, &avail),
          "file should have ended but resulted in "
          "%d more bytes",
          avail);
    if (rand() & 1) {
      CHECK(adler(fileSize) == in.getAdler32(), "bad adler32 of the read file");
    }
  }
}
