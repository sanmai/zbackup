...

{
  ...
  CHECK(header.version() == 42,
        "version is wrong when reading old header with new program");
  CHECK(header.compression_method() == "lzma",
        "compression_method is wrong when reading old header with new program");
}
}

// Write new header, read as old header
// NOTE In the real code, this will only work, if the file uses LZMA. If it
// doesn't, the version
//     field is increased and the old code will refuse to read the file.
{
...

  {
    ...
    CHECK(header.version() == 42,
          "version is wrong when reading new header with old program");
...
void readAndWrite(EncryptionKey const &key,
                  const_sptr<CompressionMethod> compression1,
                  const_sptr<CompressionMethod> compression2) {
  ...
  // read it and compare
  {
    ...
    for (int i = 0; i < chunkCount; i++) {
      ...
      CHECK(ret, "bundle.get returned false for chunk %d (%s)", i,
            chunkIds[i].c_str());
      CHECK(size == chunkSize, "wrong chunk size for chunk %d (%s)", i,
            chunkIds[i].c_str());
      CHECK(memcmp(data.c_str(), chunks[i], chunkSize) == 0,
            "wrong chunk data for chunk %d (%s)", i, chunkIds[i].c_str());
    }
  }

  // clean up
  for (int i = 0; i < chunkCount; i++) delete[] chunks[i];
  delete[] chunks;
...
int main() {
  ...
  for (CompressionMethod::iterator it = CompressionMethod::begin();
       it != CompressionMethod::end(); ++it) {
  ...
  for (size_t iteration = 100; iteration--;) {
    ...
    const_sptr<CompressionMethod> compression1 =
        compressions[rand() % compressions.size()];
    // default compression while reading the file
    // The reader should ignore it and always use the compression that was used
    // for the file.
    const_sptr<CompressionMethod> compression2 =
        compressions[rand() % compressions.size()];
