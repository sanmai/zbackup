...
UnbufferedFile::UnbufferedFile(char const *fileName,
                               Mode mode) throw(exCantOpen) {
  int flags =
      (mode == ReadWrite
           ? (O_RDWR | O_CREAT)
  ...
  if (fd < 0) throw exCantOpen(fileName);
}

size_t UnbufferedFile::read(void *buf, size_t size) throw(exReadError) {
  char *next = (char *)buf;
  size_t left = size;
  ...
  while (left) {
    ...
    if (rd < 0) {
      if (errno != EINTR) throw exReadError();
    } else if (rd > 0) {
...
void UnbufferedFile::write(void const *buf, size_t size) throw(exWriteError) {
  ...
  while (left) {
    ...
    if (written < 0) {
      if (errno != EINTR) throw exWriteError();
    } else {
...
UnbufferedFile::Offset UnbufferedFile::size() throw(exSeekError) {
  ...
  if (cur < 0) throw exSeekError();
  Offset result = lseek64(fd, 0, SEEK_END);
  if (result < 0 || lseek64(fd, cur, SEEK_SET) < 0) throw exSeekError();
  return result;
}

void UnbufferedFile::seekCur(Offset offset) throw(exSeekError) {
  if (lseek64(fd, offset, SEEK_CUR) < 0) throw exSeekError();
}

void UnbufferedFile::seek(Offset offset) throw(exSeekError) {
  if (lseek64(fd, offset, SEEK_SET) < 0) throw exSeekError();
}

UnbufferedFile::~UnbufferedFile() throw() { close(fd); }
