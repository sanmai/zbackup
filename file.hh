...
class File {
 ...
 public:
  ...
  enum OpenMode { ReadOnly, WriteOnly, Update };

  typedef long Offset;

  File(char const *filename, OpenMode) throw(exCantOpen);

  File(std::string const &filename, OpenMode) throw(exCantOpen);

  File(int fd, OpenMode) throw(exCantOpen);
  ...
  /// was used before
  size_t writeRecords(void const *buf, size_t size,
