...
void ZBackup::backupFromStdin(string const &outputFileName) {
  if (isatty(fileno(stdin))) throw exWontReadFromTerminal();
  backupFromFileHandle("stdin", stdin, outputFileName);
}

/// Backs up the data from a file
void ZBackup::backupFromFile(string const &inputFileName,
                             string const &outputFileName, bool checkFileSize) {
  File inputFile(inputFileName, File::ReadOnly);
  if (checkFileSize && inputFile.size() < config.runtime.backupMinimalSize)
    fprintf(stderr,
            "WARNING: skipping file %s because its size (use -O "
            "backup.minimalSize to adjust)\n",
            inputFileName.c_str());
  else
...
void ZBackup::backupFromDirectory(string const &inputDirectoryName,
...
void ZBackup::backupFromFileHandle(string const &inputName,
                                   FILE *inputFileHandle,
                                   string const &outputFileName) {
  if (File::exists(outputFileName)) throw exWontOverwrite(outputFileName);
...
void ZRestore::restoreToFile(string const &inputFileName,
                             string const &outputFileName) {
  ...
  BackupRestorer::restoreIterations(chunkStorageReader, backupInfo, backupData,
                                    NULL);
  ...
  struct FileWriter : public SeekableSink {
    ...
    FileWriter(UnbufferedFile *f) : f(f) {}
  ...
  BackupRestorer::restore(chunkStorageReader, backupData, NULL, NULL, &map,
                          &seekWriter);
  ...
  if (sha256.finish() != backupInfo.sha256()) throw exChecksumError();
}

void ZRestore::restoreToStdin(string const &inputFileName) {
  if (isatty(fileno(stdout))) throw exWontWriteToTerminal();
  ...
  struct StdoutWriter : public DataSink {
    ...
    virtual void saveData(void const *data, size_t size) {
      ...
      if (fwrite(data, size, 1, stdout) != 1) throw exStdoutError();
    }
  } stdoutWriter;
...
static int buse_read(void *buf, u_int32_t len, u_int64_t offset,
                     void *userdata) {
  dPrintf("NBD read offset=%lu, size=%u\n", offset, len);

  BackupRestorer::IndexedRestorer &restorer =
      *(BackupRestorer::IndexedRestorer *)userdata;
...
void ZRestore::startNBDServer(string const &inputFileName,
                              string const &nbdDevice) {
  BackupInfo backupInfo;
...
void ZExchange::exchange() {
  ...
  if (config.runtime.exchange.test(BackupExchanger::bundles)) {
    ...
    for (std::vector<string>::iterator it = bundles.begin();
         it != bundles.end(); ++it) {
      verbosePrintf("Processing bundle file %s... ", it->c_str());
      string outputFileName(Dir::addPath(dstZBackupBase.getBundlesPath(), *it));
      if (!File::exists(outputFileName)) {
        sptr<Bundle::Reader> reader = new Bundle::Reader(
            Dir::addPath(srcZBackupBase.getBundlesPath(), *it),
            srcZBackupBase.encryptionkey, true);
        sptr<Bundle::Creator> creator = new Bundle::Creator;
        sptr<TemporaryFile> bundleTempFile =
            dstZBackupBase.tmpMgr.makeTemporaryFile();
        creator->write(bundleTempFile->getFileName(),
                       dstZBackupBase.encryptionkey, *reader);

        if (creator.get() && reader.get()) {
          creator.reset();
          reader.reset();
          pendingExchangeRenames.push_back(
              BackupExchanger::PendingExchangeRename(bundleTempFile,
                                                     outputFileName));
          verbosePrintf("done.\n");
        }
      } else {
  ...
  if (config.runtime.exchange.test(BackupExchanger::indexes)) {
    ...
    for (std::vector<string>::iterator it = indexes.begin();
         it != indexes.end(); ++it) {
      verbosePrintf("Processing index file %s... ", it->c_str());
      string outputFileName(Dir::addPath(dstZBackupBase.getIndexPath(), *it));
      if (!File::exists(outputFileName)) {
        sptr<IndexFile::Reader> reader = new IndexFile::Reader(
            srcZBackupBase.encryptionkey,
            Dir::addPath(srcZBackupBase.getIndexPath(), *it));
        sptr<TemporaryFile> indexTempFile =
            dstZBackupBase.tmpMgr.makeTemporaryFile();
        sptr<IndexFile::Writer> writer = new IndexFile::Writer(
            dstZBackupBase.encryptionkey, indexTempFile->getFileName());
        ...
        if (writer.get() && reader.get()) {
          ...
          reader.reset();
          pendingExchangeRenames.push_back(
              BackupExchanger::PendingExchangeRename(indexTempFile,
                                                     outputFileName));
          verbosePrintf("done.\n");
        }
      } else {
  ...
  if (config.runtime.exchange.test(BackupExchanger::backups)) {
    ...
    for (std::vector<string>::iterator it = backups.begin();
         it != backups.end(); ++it) {
      ...
      if (!File::exists(outputFileName)) {
        ...
        BackupFile::save(tmpFile->getFileName(), dstZBackupBase.encryptionkey,
                         backupInfo);
        pendingExchangeRenames.push_back(
            BackupExchanger::PendingExchangeRename(tmpFile, outputFileName));
        verbosePrintf("done.\n");
      } else {
        verbosePrintf("file exists - skipped.\n");
      }
...
void ZCollector::gc(bool gcDeep) {
  ...
  ChunkStorage::Writer chunkStorageWriter(
      config, encryptionkey, tmpMgr, chunkReindex, getBundlesPath(),
      getIndexPath(), config.runtime.threads);

  string fileName;

  BundleCollector collector(getBundlesPath(), &chunkStorageReader,
                            &chunkStorageWriter, gcDeep, config);
  ...
  for (std::vector<string>::iterator it = backups.begin(); it != backups.end();
       ++it) {
    ...
    BackupRestorer::restoreIterations(chunkStorageReader, backupInfo,
                                      backupData, &collector.usedChunkSet);

    BackupRestorer::restore(chunkStorageReader, backupData, NULL,
                            &collector.usedChunkSet, NULL, NULL);
  }
...
void ZInspect::inspect(string const &inputFileName) {
  ...
  if (chunkIndex.size()) {
    ...
    ChunkStorage::Reader chunkStorageReader(config, encryptionkey, chunkIndex,
                                            getBundlesPath(),
                                            config.runtime.cacheSize);
    string backupData;
    BackupRestorer::restoreIterations(chunkStorageReader, backupInfo,
    ...
    for (BackupRestorer::ChunkMap::const_iterator it = map.begin();
         it != map.end(); it++) {
